...
import {
  ...
  WEBSITE_FORM_SMS_BODY_STARTS_WITH,
...
export function isLeadInboxToContact(item: Pick<WebsiteLeadInboxItem, "leadStatus">) {
...
export async function listWebsiteLeadInboxItems(companyId: string): Promise<WebsiteLeadInboxItem[]> {
  const [emailLeads, smsLeadMessages] = await Promise.all([
    ...
    prisma.message.findMany({
      where: {
        ...
        body: { startsWith: WEBSITE_FORM_SMS_BODY_STARTS_WITH },
        conversation: {
          companyId,
  ...
  return [...emailItems, ...smsItems]
    .filter((item) => {
      if (item.source?.toLowerCase() === "careers") return false;
      const lead = item.leadId ? leadsById.get(item.leadId) : null;
...
export async function countLeadsToContact(companyId: string) {
  const items = await listWebsiteLeadInboxItems(companyId);
  return items.filter(isLeadInboxToContact).length;
}

export { isWebsiteFormSmsBody, WEBSITE_FORM_SMS_BODY_STARTS_WITH } from "@/lib/inbox/website-leads";
