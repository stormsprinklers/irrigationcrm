...
import { issuePhoneReleaseActionToken } from "@/lib/twilio/phone-release-token";
...
/** Start / verify step-up MFA before releasing Twilio phone numbers. */
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "start") {
      const result = await startStaffMfaChallenge(user, AuthMfaPurpose.PHONE_NUMBER_RELEASE);
      if (!result.ok) {
    ...
    if (action === "verify") {
      ...
      const verified = await verifyStaffMfaChallenge(
        challengeId,
        code,
        AuthMfaPurpose.PHONE_NUMBER_RELEASE
      );
      if (!verified.ok) {
      ...
      const actionToken = await issuePhoneReleaseActionToken({
        userId: user.id,
