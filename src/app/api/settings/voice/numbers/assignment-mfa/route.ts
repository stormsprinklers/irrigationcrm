...
import { issuePhoneStepUpToken, PHONE_ASSIGNMENT_PURPOSE } from "@/lib/twilio/phone-release-token";
...
async function requireManager(): Promise<StaffAuthUser> {
...
/** Start / verify step-up MFA before changing locked company/campaign assignments. */
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "start") {
      const result = await startStaffMfaChallenge(user, AuthMfaPurpose.PHONE_ASSIGNMENT_UNLOCK);
      if (!result.ok) {
    ...
    if (action === "verify") {
      ...
      const verified = await verifyStaffMfaChallenge(
        challengeId,
        code,
        AuthMfaPurpose.PHONE_ASSIGNMENT_UNLOCK
      );
      if (!verified.ok) {
      ...
      const actionToken = await issuePhoneStepUpToken({
        userId: user.id,
