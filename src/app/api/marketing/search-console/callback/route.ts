...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const redirectUri = `${appUrl}/api/marketing/search-console/callback`;
    ...
    await prisma.company.update({
      ...
      data: {
        googleSearchConsoleRefreshToken:
          tokens.refresh_token ?? existing.googleSearchConsoleRefreshToken,
        googleSearchConsoleConnectedAt: new Date(),
      },
    });
