...
import { exchangeOAuthCode, verifyOAuthState } from "@/lib/google-ads/client";
import { prisma } from "@/lib/prisma";

export async function GET(request: NextRequest) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? request.nextUrl.origin;
  const redirectBase = `${appUrl}/settings/integrations/google-ads`;
  ...
  try {
    ...
    await prisma.company.update({
      where: { id: companyId },
      data: {
        googleAdsRefreshToken: tokens.refresh_token ?? existing.googleAdsRefreshToken,
